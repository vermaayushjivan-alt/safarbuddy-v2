...
import { getAllDestinationsAdmin, deleteDestinationAdmin } from "@/app/actions/destination.actions";
...
export default async function AdminDestinationsListPage({
  ...
  const { data: destinations, total, totalPages, hasNext, hasPrev } =
    await getAllDestinationsAdmin(page, 20);
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12">
      <div className="mb-8 flex items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl text-deep">Destinations</h1>
        ...
        <table className="w-full text-left text-[13px]">
          <thead className="border-b border-deep/10 bg-mist text-[11px] uppercase tracking-wide text-ink/50">
            <tr>
              ...
              <th className="px-4 py-3 font-heading font-semibold">State</th>
              <th className="px-4 py-3 font-heading font-semibold">Status</th>
              <th className="px-4 py-3 font-heading font-semibold">Featured</th>
          ...
          <tbody>
            {destinations.length === 0 ? (
              ...
              destinations.map((destination) => (
                <tr key={destination.id} className="border-b border-deep/10 last:border-0">
                  <td className="px-4 py-3 font-medium text-deep">
                    {destination.name}
                  </td>
                  <td className="px-4 py-3 text-ink/70">{destination.state ?? "—"}</td>
      ...
      {totalPages > 1 && (
        <div className="mt-6 flex items-center justify-center gap-3">
          <Link
            href={`/admin/destinations?page=${page - 1}`}
