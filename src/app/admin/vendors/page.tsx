...
import { getAllVendorsAdmin, deleteVendorAdmin } from "@/app/actions/vendor.actions";
...
export default async function AdminVendorsListPage({
  ...
  const { data: vendors, total, totalPages, hasNext, hasPrev } =
    await getAllVendorsAdmin(page, 20);

  async function handleDelete(formData: FormData) {
    "use server";
    const id = formData.get("id") as string;
    const result = await deleteVendorAdmin(id);
    if (!result.success) {
      // No client-side error UI in this fire-and-forget delete form;
      // log server-side so a failed delete is diagnosable.
      console.error("[deleteVendorAdmin]", result.error);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12">
      <div className="mb-8 flex items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl text-deep">Vendors</h1>
