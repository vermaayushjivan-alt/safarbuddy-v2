...
import {
  getMyHotelImages,
  uploadMyHotelImage,
  setMyPrimaryHotelImage,
  reorderMyHotelImage,
  deleteMyHotelImage,
  type OwnerHotelImageWithUrl,
} from "@/app/actions/owner-hotel-image.actions";

interface OwnerHotelImageManagerProps {
  hotelId: string;
}

export function OwnerHotelImageManager({
  hotelId,
}: OwnerHotelImageManagerProps) {
  const [images, setImages] = useState<OwnerHotelImageWithUrl[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  async function refresh() {
    const result = await getMyHotelImages(hotelId);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setImages(
          [...result.data].sort((a, b) => a.sort_order - b.sort_order)
        );
      } catch (err) {
        if (!active) return;
  ...
  function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      try {
        const result = await uploadMyHotelImage(
          hotelId,
          file,
          images.length === 0
        );
  ...
  function handleSetPrimary(imageId: string) {
    ...
    startTransition(async () => {
      try {
        const result = await setMyPrimaryHotelImage(hotelId, imageId);
  ...
  function handleSortOrderChange(imageId: string, sortOrder: number) {
    ...
    startTransition(async () => {
      try {
        const result = await reorderMyHotelImage(hotelId, imageId, sortOrder);
  ...
  function handleDelete(imageId: string) {
    ...
    startTransition(async () => {
      try {
        const result = await deleteMyHotelImage(hotelId, imageId);
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-[12px] text-ink/45">
          jpg, jpeg, png, or webp. Max 5MB. These photos (exterior, lobby,
          pool, etc.) are what guests see in your property&apos;s gallery —
          not individual room photos.
        </p>
      </div>

      {loading ? (
        <p className="text-[13px] text-ink/50">Loading photos...</p>
      ) : images.length === 0 ? (
        <p className="text-[13px] text-ink/50">No photos uploaded yet.</p>
      ) : (
